...
out_dir_1 <- paste0(out_dir_1, 'tables/')
dir.create(out_dir_1)
...
v_dependents <- c(
  ...
  'functional_ch', 'one_ch_muts_functional', 'more_than_one_ch_muts_functional',
   ...
   'ch_cnv', 'ch_amp', 'ch_del', 'ch_loh',
	...
	# if (CONDITION=='germ_mutated_ex'){
	# 	data2 = data %>% filter(!n_germ_muts>1)
	# 	CONDITION='germ_mutated'
	# }
	...
	for (j in 1:length(v_dependents)){
		...
		# b = c(rep(NA, 2))
		# names(b) <- c('glm.odds-ratio', 'glm.p.value')
		b = c(rep(NA, 4))
		names(b) <- c('glm.odds-ratio', 'glm.p.value', 'glm.conf.low', 'glm.conf.high')

		if ( DEPENDENT %in% paste0('functional_ch__', MajorCancerTypes_cols)) {

			Glm.test <- glm(as.factor(get(DEPENDENT)) ~ get(CONDITION)+age_bin+Sex+ancestry_label+smoking_bin+binary_therapy , data = data2, family = "binomial")
...
## Export:
	NAME <- paste(out_dir_1, 'MainAssociations_by._', CONDITION, '_.tsv', sep='')
